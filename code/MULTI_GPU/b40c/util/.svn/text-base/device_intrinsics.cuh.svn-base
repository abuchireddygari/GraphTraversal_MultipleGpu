...
//    }
    ...
    __device__ __forceinline__ int TallyWarpVote_BFS(int predicate)
    {
#if __CUDA_ARCH__ >= 200
      ...
      int tid = threadIdx.x & (B40C_WARP_THREADS_BFS(__B40C_CUDA_ARCH__) - 1);
      int wid = threadIdx.x >> B40C_LOG_WARP_THREADS_BFS(__B40C_CUDA_ARCH__);
    ...
    __device__ __forceinline__ int TallyWarpVote_BFS(
                                                     int predicate,
     ...
     */
    template <int LOG_ACTIVE_WARPS, int LOG_ACTIVE_THREADS>
    __device__ __forceinline__ int WarpVoteAll_BFS(int predicate)
    {
#if __CUDA_ARCH__ >= 120
      return __all(predicate);
#else 
      const int ACTIVE_THREADS = 1 << LOG_ACTIVE_THREADS;
      return (TallyWarpVote_BFS<LOG_ACTIVE_WARPS, LOG_ACTIVE_THREADS>(predicate) == ACTIVE_THREADS);
#endif
    }
    ...
    __device__ __forceinline__ int WarpVoteAll_BFS(int predicate)
    {
      ...
      return (TallyWarpVote_BFS<LOG_ACTIVE_THREADS>(predicate, storage) == ACTIVE_THREADS);
#endif
    }
...
#else
      return TallyWarpVote_BFS<LOG_ACTIVE_WARPS, LOG_ACTIVE_THREADS>(predicate);
#endif
    }
    ...
    __device__ __forceinline__ int WarpVoteAny(int predicate)
    {
      ...
      return TallyWarpVote_BFS<LOG_ACTIVE_THREADS>(predicate, storage);
#endif
    }
